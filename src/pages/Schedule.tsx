    ...
    <div className="flex flex-col h-full overflow-hidden bg-[#0B1120]">
              ...
              <div className="flex relative" style={{ minWidth: viewMode === "week" ? `${days.length * 120}px` : "100%", height: 24 * HOUR_HEIGHT }}>
                ...
                {days.some((d) => isSameDay(d, new Date())) && (
                  <div className="absolute left-0 right-0 z-40 pointer-events-none" style={{ top: currentTimeTop }}>
                    <div className="h-[2px] bg-gradient-to-r from-[#00E5CC] via-[#00E5CC]/60 to-[#00E5CC]/20 shadow-[0_0_8px_rgba(0,229,204,0.5)]" />
                  </div>
                )}
                {days.map((day, dayIdx) => {
                  const dayBlocks = expandedBlocks.filter((b) => isSameDay(new Date(b.start_at), day));
      ...
      {(touchDragItem || touchDragPlaylist) && touchDragPos && (
        ...
        >
          ...
          {/* Time label tooltip */}
          {touchDropHighlight && (() => {
            const totalMinutes = touchDropHighlight.hour * 60 + Math.round((touchDropHighlight.offsetY / HOUR_HEIGHT) * 60);
